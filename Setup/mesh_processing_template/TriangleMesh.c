...
void computeVertexNormals(TriangleMesh* tri_mesh) {
	...
	//P2-P1
	Vector3 subone;
	sub(tri_mesh->_vertex_normals[v1], tri_mesh->_vertex_normals[v0], &subone);
	//P3-P1
	Vector3 subtwo;
	sub(tri_mesh->_vertex_normals[v2], tri_mesh->_vertex_normals[v0], &subtwo);
	//(P2-P1)*(P3-P1)
	Vector3 cross;
	computeCrossProduct(subone, subtwo, &cross);
	//euclid norm
	float area;
...
void heatStep(TriangleMesh* tri_mesh) {
  ...
  //L(vi) = (1/number of vertics adjacent) * (sum of vertics adjacent) - vi
  ...
  // Get the number of vertices adjacent to the vertex i
   Vector3 sum_neighbors;
  
  for(int i; i<tri_mesh->_number_vertices;i++)
    {
	...
	add(pj,sum_neighbors,&(sum_neighbors));
      } 
      mulAV(1/(float)num_neighbors, sum_neighbors,&(sum_neighbors));
      sub(sum_neighbors,tri_mesh->_vertices[i],&(sum_neighbors));
      
      mulAV(0.5, sum_neighbors,&(sum_neighbors));
      
      add(tri_mesh->_vertices[i],sum_neighbors,&(tri_mesh->_vertices[i]));
    }
...
void computeBoundaryEdges(TriangleMesh* mesh) {
  ...
  mesh->_edges = (Pair*)malloc(sizeof(Pair) * mesh->_number_triangles *3);
  
  for(int i=0;i<mesh->_number_triangles;i++)
    {
      int triangle_points[3];
      triangle_points[0] = mesh->_triangles[i]._v0;
      triangle_points[1] = mesh->_triangles[i]._v1;
      triangle_points[2] = mesh->_triangles[i]._v2;
      int matched_vertex = 0;
      int next1 = getNumberAdjacentVertices(mesh, triangle_points[0]);
      int next2 = getNumberAdjacentVertices(mesh, triangle_points[1]);
      for(int j=0;j<next1;j++)
	{
	  int numberone = getAdjacentVertex(mesh, triangle_points[0], j);
	  for(int k=0;k<next2;k++)
	    {
	      int numbertwo = getAdjacentVertex(mesh, triangle_points[1], k);
      ...
      if(matched_vertex == 1)
	{
	  mesh->_edges[mesh->_number_edges]._v0 = triangle_points[0];
	  mesh->_edges[mesh->_number_edges]._v1 = triangle_points[1];
	  mesh->_number_edges++;
	}
      matched_vertex = 0;
      next1 = getNumberAdjacentVertices(mesh, triangle_points[1]);
      next2 = getNumberAdjacentVertices(mesh, triangle_points[2]);
      for(int j=0;j<next1;j++)
	{
	  int numberone = getAdjacentVertex(mesh, triangle_points[1], j);
	  for(int k=0;k<next2;k++)
	    {
	      int numbertwo = getAdjacentVertex(mesh, triangle_points[2], k);
	      if(numberone == numbertwo)
		{
		  matched_vertex++;
		}
      ...
      if(matched_vertex == 1)
	{
	  mesh->_edges[mesh->_number_edges]._v0 = triangle_points[1];
	  mesh->_edges[mesh->_number_edges]._v1 = triangle_points[2];
	  mesh->_number_edges++;
	}
      matched_vertex = 0;
      next1 = getNumberAdjacentVertices(mesh, triangle_points[0]);
      next2 = getNumberAdjacentVertices(mesh, triangle_points[2]);
      for(int j=0;j<next1;j++)
	{
	  int numberone = getAdjacentVertex(mesh, triangle_points[0], j);
